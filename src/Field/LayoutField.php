...
namespace Joomla\Plugin\RadicalMart\Related\Field;
		...
		if ($plugin && $client)
		{
			...
			if (is_dir($plugin_path) && ($plugin_layouts = Folder::files($plugin_path, '^[^_]*\.php$')))
			{
				...
				foreach ($plugin_layouts as $file)
				{
					...
					$groups['_']['items'][] = HTMLHelper::_('select.option', '_:' . $value, ucfirst($value));
				}
			}
					...
					if (is_dir($template_path) && ($files = Folder::files($template_path, '^[^_]*\.php$')))
					{
						...
						if (\count($files))
						{
							...
							foreach ($files as $file)
							{
								...
								$value                                 = basename($file, '.php');
								$groups[$template->element]['items'][] = HTMLHelper::_('select.option', $template->element . ':' . $value, ucfirst($value));
							}
						}
					}
				}
